...
import tertiaryCategoryReducer from "../TertiaryCategory/reducer";



class EvCharges extends Component {
  constructor(props) {
    super(props);
    this.state ={
...
async componentDidMount(){
  ...
  const pageidofterms = this.props.pagesid.pages["ev-chargers"].id;
  ...
  let headerTitle = "EV Charges"
    const { body, title, headerImage,pagesid,imageurl} = this.props;
     return (
      ...
      showMessagee = {showMessagee} headerTitle ={headerTitle}
      handleCloseModal = {this.handleCloseModal}  headerImage={ imageurl || headerImage}
       saveFile = {this.handleFile} title={title} />
    );
  }
}
