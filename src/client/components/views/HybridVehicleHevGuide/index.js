...
class HybridVehicleHevGuide extends Component {
...
async componentDidMount(){
  ...
  const pageidofterms = this.props.pagesid.pages["hybrid-vehicle-hev-guide"].id;
...
handleFile = async(file) =>{
 ...
 render() {
  ...
  let headerTitle = "hybrid-vehicle-hev-guide"
    const { body, title, headerImage,pagesid,imageurl} = this.props;
   return (
      <Editor body = {body || undefined} saveData={this.handleSave} isOpen={showModalForImagee}
      showMessagee = {showMessagee} headerTitle ={headerTitle} headerImage={ imageurl || headerImage}
      handleCloseModal = {this.handleCloseModal}  saveFile = {this.handleFile} title={title} />
    );
  }
}
