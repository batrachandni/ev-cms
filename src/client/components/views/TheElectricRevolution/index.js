...
import {getAllPagesId, getPageDataById,sendHtmlData,sendImageFile} from './action'
import Editor from "../../General/Editor";


class TheElectricRevolution extends Component {
  constructor(props) {
    super(props);
    this.state ={
...
async componentDidMount(){
  ...
  const pageidofterms = this.props.pagesid.pages["the-electric-revolution"].id;
...
handleFile = async(file) =>{
 ...
 render() {
    ...
    let headerTitle = "the-electric-revolution" 
    const { body, title, headerImage,pagesid,imageurl} = this.props;
       return (
     <Editor body = { body || undefined } saveData={this.handleSave} showMessagee = {showMessagee} 
      headerImage={ imageurl || headerImage} saveFile = {this.handleFile} title={title || undefined} 
      isOpen={showModalForImagee} headerTitle ={headerTitle} imageUrl = {this.handleFile} 
